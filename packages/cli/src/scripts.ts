...
export function generateMigrationRunnerScript(
  ...
  const orderedModuleVars = ordered.map((i) => i.moduleVar);
  const migrateLoop =
    orderedModuleVars.length > 0
      ? orderedModuleVars
          .map((v) => `  await ${v}.migrate();`)
          .join("\n")
      : "  // no collections";

  // Emitted run-all.migrate.js: runs each collection's migrate() in dependency order
  return (
    `// Auto-generated by sql2nosql.\n` +
    `// Runs all per-collection migrations in dependency order.\n` +
    `// Usage: node run-all.migrate.js\n\n` +
    `${imports}\n\n` +
    `async function runAllMigrations() {\n` +
    `  const order = [${ordered.map((i) => `"${i.name}"`).join(", ")}];\n` +
    `  console.log("Running migrations in order:", order.join(" → "));\n` +
    `${migrateLoop}\n` +
